...
export async function main(ns: NS) {
  ...
  ns.tprintf(
    `* Grow Threads for 10%%: ${server.growNeeded10per} (${ns.tFormat(server.growthTime * server.growNeeded10per)})`
  );
  ns.tprintf(
    `* Grow Threads for 50%%: ${server.growNeeded50per} (${ns.tFormat(server.growthTime * server.growNeeded50per)})`
  );
  ...
  ns.tprintf(
    `${server.hackSecurity > server.weakenPerThread ? COLORS.RED : COLORS.GREEN}* Security Increase with Hack: ${server.hackSecurity}`
  );
}
