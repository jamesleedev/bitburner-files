...
export function getHostFlag(ns: NS, cmdFlags: CmdFlags, includePositional: boolean = false, printError: boolean = false): string | null {
  const filter = includePositional ? ['_', 'h', 'host'] : ['h', 'host'];

  let hostFlags = Object.entries(cmdFlags).filter((flag) => filter.includes(flag[0]));


  let hostArgs = hostFlags.map((flag) => {
    const arg = flag[1];
